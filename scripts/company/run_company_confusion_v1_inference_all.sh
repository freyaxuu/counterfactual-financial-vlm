...
# company_confusion_v1 unique-question index, 4-at-a-time (one per GPU), in
# 3 rounds. Meant to run detached (nohup ... &) on the training server so it survives
# SSH disconnect. Mirrors scripts/run_tatqa_natural_pairs_inference_all.sh's
# structure and reuses the exact same 9 checkpoint adapter paths already
# selected via TAT-QA dev-set validation -- per AGENTS.md, company data must
# never be used to select checkpoints, so this script does not introduce any
# new checkpoint choice.
set -uo pipefail

REPO=~/apps/cf-vlm-financial-grounding
RUNS=~/runs/cf-vlm-financial-grounding
INDEX=$RUNS/company_confusion_v1_unique_questions/unique_questions.jsonl
OUT_ROOT=$RUNS/company_confusion_v1_predictions
mkdir -p "$OUT_ROOT"
