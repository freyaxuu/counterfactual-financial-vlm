#!/usr/bin/env python3
"""Train CF augmentation + Evidence-first Grounding LoRA.

Follow-up named explicitly in the Evidence-first Grounding spec's closing
note: test whether explicit bbox-before-answer localization adds value once
counterfactual re-binding examples are present, "using exactly the same CF
examples and schedule as the existing CF-augmentation baseline."

Same shuffled-balanced CF cycle as CF-augmentation (train_qwen3_vl_cf_lora.py):
every group visited 3 times per cycle, once per CF type (target-value
replacement, header/address swap, irrelevant-value replacement), each visit
pairing clean + one CF full-page image. The difference is the *target format*
on each side: instead of plain-QA (`gold_answer` only), both sides use the
Evidence-first bbox-before-answer target

    <bbox>x1 y1 x2 y2</bbox>
    <answer>...</answer>

resolved independently from each side's own evidence block (clean's bbox for
the clean side, the CF variant's own bbox/answer for the counterfactual
side). Loss is the same summed dual-forward-pass form as CF-augmentation:

    L = L_evidence_first(clean) + L_evidence_first(counterfactual)

No row/column supervision (matching Evidence-first Grounding, not
Grounding-only). Uses financial_vlm.training.cf_evidence_first_grounding,
which composes cf_cycle_sampler (group/cf_type scheduling) and
grounding_target (bbox/answer resolution + evidence-first serialization)
without modifying either. train_qwen3_vl_lora.py, train_qwen3_vl_cf_lora.py,
train_grounding_lora.py, train_evidence_first_grounding_lora.py are
untouched.

Groups must pass grounding validation on clean AND all 3 CF variants (a
group can draw any CF type on any visit), so the valid-group set here is a
subset of Grounding-only/Evidence-first's 995 clean-only-valid groups --
see validate_and_filter_groups. Never silently drops a group without
counting it.

Smoke test / overfit sanity check: run with --limit-groups 20-30 and a large
--num-cycles override (each visit is already one full sample, so cycles are
the lever here, not epochs -- num_train_epochs is fixed at 1.0 since the
materialized pair list already IS one full pass). Then run
eval_evidence_first_grounding_lora.py --limit-groups <same N> --print-samples
10 against the resulting adapter -- the eval script needs no changes since
the output format is identical to plain Evidence-first Grounding.
"""
...
DEFAULT_CONFIG = REPO_ROOT / "configs" / "synfintabs" / "synfintabs_train_dev_test_ood_cfv1_cf_evidence_first_grounding_lora.yaml"
...
def prepare_output_dir(path: Path, config_path: Path, config: Mapping[str, Any]) -> Path:
    ...
    metadata = {
        ...
        "baseline": "CF-augmentation + Evidence-first Grounding: same shuffled-balanced CF cycle as "
        "CF-augmentation, both sides use the bbox-before-answer target",
        "objective": "L = L_evidence_first(clean) + L_evidence_first(counterfactual)",
    ...
    messages: list[dict[str, Any]] = [
        {
            ...
            "content": [
                ...
                {"type": "text", "text": evidence_first_prompt(question)},
            ],
        }
    ]
...
class CFEvidenceFirstPairDataset:
    """One example per shuffled-balanced-cycle pair: {clean, counterfactual}.

    Each side is a GroundingExample (build_grounding_example already
    resolved its own bbox/answer/target_cell_id); the training target string
    is rebuilt via format_evidence_first_target (bbox-before-answer), not
    example.target_text (that's the 4-field Grounding-only format).
    """
    ...
    def _build_example(self, example: GroundingExample) -> dict[str, Any]:
        ...
        target = format_evidence_first_target(example.answer_raw, example.bbox_1000)
        ...
        full_inputs = self.processor.apply_chat_template(
            build_messages(image, example.question, target),
            tokenize=True,
...
def main() -> int:
    ...
    train_dataset = CFEvidenceFirstPairDataset(pairs, data_root, processor)
