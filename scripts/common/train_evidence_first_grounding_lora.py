#!/usr/bin/env python3
"""Train the Evidence-first Grounding LoRA baseline -- ablation of Grounding-only.

Grounding-only (train_grounding_lora.py) put <answer> before <row>/<column>/
<bbox> in the assistant target. Because generation is autoregressive, the
answer token could never actually be conditioned on the bbox that came after
it -- so that experiment couldn't test whether grounding *causally* helps the
answer, only whether the model can emit a plausible-looking bbox once it
already knows the answer. This ablation forces the opposite order and drops
row/column entirely (they're often copyable straight out of the question
text without any visual grounding, which would make row/column accuracy an
uninformative signal here):

    <bbox>x1 y1 x2 y2</bbox>
    <answer>...</answer>

Same clean-only training data as Grounding-only: the identical 995 valid
groups (no counterfactuals, no oracle packets, no highlighted/cropped
images, no CF-pair or consistency loss), same LR (2e-4), same seed
(20260804), same LoRA/training hyperparameters -- single-seed sanity
experiment, no LR sweep. Loss is the ordinary causal-LM loss over the new
target string (prompt masked), no separate grounding loss term.

Reuses validate_and_build_examples / check_split_leakage / build_grounding_example
from financial_vlm.data.grounding_target unchanged (same data, same
validation, same excluded 5 ambiguous-header groups) -- only the serialized
target string and prompt differ, via format_evidence_first_target /
evidence_first_prompt. train_grounding_lora.py, train_qwen3_vl_lora.py,
train_qwen3_vl_cf_lora.py are untouched.

Smoke test / overfit sanity check: run with --limit-groups 20-50 and
--num-train-epochs 40 (batch_size=1/grad_accum=16 barely reaches any
optimizer steps on a tiny subset at the production epoch count), then run
eval_evidence_first_grounding_lora.py --limit-groups <same N> --print-samples 10
against the resulting adapter before launching the full run.
"""
...
DEFAULT_CONFIG = REPO_ROOT / "configs" / "synfintabs" / "synfintabs_train_dev_test_ood_cfv1_evidence_first_grounding_lora.yaml"
TRAIN_VARIANT = "clean"
...
def prepare_output_dir(path: Path, config_path: Path, config: Mapping[str, Any]) -> Path:
    ...
    metadata = {
        ...
        "baseline": "Evidence-first Grounding LoRA: bbox generated before answer, clean images only, no CF data",
        "objective": "causal_lm_loss(<bbox>x1 y1 x2 y2</bbox><answer>...</answer>)",
...
def print_validation_summary(stats: Mapping[str, int]) -> None:
    print("=== Grounding validation summary (shared with Grounding-only: same 995 valid groups) ===")
...
class EvidenceFirstGroundingDataset:
    ...
    def __getitem__(self, index: int) -> dict[str, Any]:
        ...
        # bbox-before-answer target -- ignores example.target_text (that's the
        # 4-field Grounding-only format), builds the 2-field evidence-first one.
...
def main() -> int:
    ...
    train_dataset = EvidenceFirstGroundingDataset(examples, data_root, processor)
